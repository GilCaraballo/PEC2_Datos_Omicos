...
filter_microarray <- function(allTargets, seed = 43141876) {
  ...
  filtered <- subset(allTargets, hora != "hour 2")
...
```{r}
# Clustering jerárquico con datos crudos
library(stats)
...
plot(clust.euclid.average, labels = result$muestra, 
...
plot(clust_norm, labels = sampleNames(normalized_data),
...
```{r}
# Filtrar las sondas
# Ejecutar nsFilter sin Entrez ni cuantiles
...
```
```{r}
# Obtener resultados con filtro en logFC y p-value ajustado
results_untreated <- topTable(
  fit2,
  coef = "Infected_vs_Uninfected_Untreated",
  number = nrow(fit2),
  adjust = "fdr",  # Ajuste de p-value usando FDR
  lfc = 3,         # Umbral para log-fold-change
  p.value = 0.05   # Umbral para p-value ajustado
)

# Revisar las dimensiones del resultado
dim(results_untreated)
